...
typedef struct __mavlink_thread_info_t
{
 ...
 char name[10]; ///< User readable thread name.
 uint8_t priority; ///< Get the priority of the thread.
} mavlink_thread_info_t;

#define MAVLINK_MSG_ID_THREAD_INFO_LEN 32
#define MAVLINK_MSG_ID_2_LEN 32

#define MAVLINK_MSG_ID_THREAD_INFO_CRC 2
#define MAVLINK_MSG_ID_2_CRC 2

#define MAVLINK_MSG_THREAD_INFO_FIELD_NAME_LEN 10
...
#define MAVLINK_MESSAGE_INFO_THREAD_INFO { \
	...
	{  { "time", NULL, MAVLINK_TYPE_UINT32_T, 0, 0, offsetof(mavlink_thread_info_t, time) }, \
         ...
         { "name", NULL, MAVLINK_TYPE_CHAR, 10, 21, offsetof(mavlink_thread_info_t, name) }, \
         { "priority", NULL, MAVLINK_TYPE_UINT8_T, 0, 31, offsetof(mavlink_thread_info_t, priority) }, \
         } \
}
 ...
 * @param ID Thread ID.
 * @param name User readable thread name.
...
static inline uint16_t mavlink_msg_thread_info_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint32_t time, uint8_t ID, const char *name, uint8_t priority, uint16_t duration, uint16_t latency, uint32_t total_duration, uint32_t total_latency, uint32_t number_of_executions)
{
	...
	_mav_put_uint8_t(buf, 31, priority);
	_mav_put_char_array(buf, 21, name, 10);
...
{
	return mavlink_msg_thread_info_pack(system_id, component_id, msg, thread_info->time, thread_info->ID, thread_info->name, thread_info->priority, thread_info->duration, thread_info->latency, thread_info->total_duration, thread_info->total_latency, thread_info->number_of_executions);
}
...
{
	return mavlink_msg_thread_info_pack_chan(system_id, component_id, chan, msg, thread_info->time, thread_info->ID, thread_info->name, thread_info->priority, thread_info->duration, thread_info->latency, thread_info->total_duration, thread_info->total_latency, thread_info->number_of_executions);
}
...
static inline void mavlink_msg_thread_info_send(mavlink_channel_t chan, uint32_t time, uint8_t ID, const char *name, uint8_t priority, uint16_t duration, uint16_t latency, uint32_t total_duration, uint32_t total_latency, uint32_t number_of_executions)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[MAVLINK_MSG_ID_THREAD_INFO_LEN];
	_mav_put_uint32_t(buf, 0, time);
 ...
 */
static inline void mavlink_msg_thread_info_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint32_t time, uint8_t ID, const char *name, uint8_t priority, uint16_t duration, uint16_t latency, uint32_t total_duration, uint32_t total_latency, uint32_t number_of_executions)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
/**
 * @brief Get field name from thread_info message
 *
 * @return User readable thread name.
 */
static inline uint16_t mavlink_msg_thread_info_get_name(const mavlink_message_t* msg, char *name)
{
	return _MAV_RETURN_char_array(msg, name, 10,  21);
}
...
static inline void mavlink_msg_thread_info_decode(const mavlink_message_t* msg, mavlink_thread_info_t* thread_info)
{
	...
	mavlink_msg_thread_info_get_name(msg, thread_info->name);
