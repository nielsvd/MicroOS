...
typedef struct __mavlink_heartbeat_t
{
 ...
 uint32_t time; ///< System status flag, see MAV_STATE ENUM
 uint32_t event; ///< Send that an event has occurred.
 ...
 * @param time System status flag, see MAV_STATE ENUM
 * @param VBat Battery voltage
 * @param event Send that an event has occurred.
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_heartbeat_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint8_t type, uint32_t mode, uint32_t time, int16_t VBat, uint32_t event)
{
...
{
	return mavlink_msg_heartbeat_pack(system_id, component_id, msg, heartbeat->type, heartbeat->mode, heartbeat->time, heartbeat->VBat, heartbeat->event);
}
...
{
	return mavlink_msg_heartbeat_pack_chan(system_id, component_id, chan, msg, heartbeat->type, heartbeat->mode, heartbeat->time, heartbeat->VBat, heartbeat->event);
}
...
static inline void mavlink_msg_heartbeat_send(mavlink_channel_t chan, uint8_t type, uint32_t mode, uint32_t time, int16_t VBat, uint32_t event)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[MAVLINK_MSG_ID_HEARTBEAT_LEN];
	_mav_put_uint32_t(buf, 0, mode);
 ...
 */
static inline void mavlink_msg_heartbeat_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t type, uint32_t mode, uint32_t time, int16_t VBat, uint32_t event)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
 ...
 *
 * @return Send that an event has occurred.
 */
